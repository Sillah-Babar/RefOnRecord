"""
Resume Verifier API — SQLAlchemy ORM Models
...
- ResumeProject: a resume instance owned by a user
- Experience: a work history entry within a resume project
- VerificationRequest: a pending or completed request to authenticate a work experience
- ShareLink: a secure, shareable access token for a resume project

Authentication is handled via stateless JWT tokens (see auth.py), so there is no
...
class User(db.Model):
    ...
    A user can own multiple resume projects. Authentication is stateless (JWT);
    no server-side session rows are stored.
    """
    __tablename__ = "user"
    ...
    def serialize(self):
        ...
        return {
            ...
            "links": {
                "self": url_for("api.user_resource", user=self),
                "projects": url_for("api.user_project_collection", user=self),
            },
        }
...
class ResumeProject(db.Model):
    ...
    def serialize(self):
        ...
        return {
            ...
            "links": {
                "self": url_for("api.project_resource", user=self.owner, project=self),
                "owner": url_for("api.user_resource", user=self.owner),
                "experiences": url_for("api.project_experience_collection", user=self.owner, project=self),
                "shares": url_for("api.project_share_collection", user=self.owner, project=self),
            },
        }
    ...
    def serialize(self):
        ...
        return {
            ...
            "links": {
                ...
                "project": url_for(
                    "api.project_resource",
                    user=self.project.owner,
                    project=self.project,
                ),
                "verification_requests": url_for(
                    "api.experience_verification_collection",
                    user=self.project.owner,
                    project=self.project,
                    experience=self,
                ),
        ...
        return {
            ...
            "links": {
                "self": url_for(
                    ...
                    user=self.experience.project.owner,
                    project=self.experience.project,
                ...
                "respond": url_for("api.verification_respond", request_id=self.request_id),
            },
        }
...
class ShareLink(db.Model):
    ...
    def serialize(self):
        ...
        return {
            ...
            "links": {
                "self": url_for("api.public_share", share_token=self.share_token),
                "project": url_for("api.project_resource", user=self.project.owner, project=self.project),
            },
        }
